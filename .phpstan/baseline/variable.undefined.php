<?php declare(strict_types = 1);

$ignoreErrors = [];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../Documentation/help_files/sl_eob_help.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$config might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../admin.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$database_acl might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../admin.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$database_patch might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../admin.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$dbase might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../admin.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$host might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../admin.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$login might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../admin.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pass might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../admin.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$port might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../admin.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_acl might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../admin.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_database might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../admin.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_realpatch might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../admin.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$authorID might not be defined\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../ccr/createCCRActor.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$oemrID might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../ccr/createCCRActor.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sourceID might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../ccr/createCCRAlerts.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$authorID might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../ccr/createCCRHeader.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sourceID might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../ccr/createCCRImmunization.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sourceID might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../ccr/createCCRMedication.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sourceID might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../ccr/createCCRProblem.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sourceID might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../ccr/createCCRProcedure.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$authorID might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../ccr/createCCRResult.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$policies_by_payer_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../contrib/util/chart_review_pids.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$dupecount might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../contrib/util/dupecheck/index.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$dupelist might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../contrib/util/dupecheck/index.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sqlconf might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../contrib/util/dupecheck/mergerecords.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../contrib/util/dupscore.cli.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$d might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../controllers/C_Document.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$error might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../controllers/C_Document.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$filetext might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../controllers/C_Document.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$from_pathname might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../controllers/C_Document.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$messages might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../controllers/C_Document.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$temp_couchdb_url might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../controllers/C_Document.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$p might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../controllers/C_Prescription.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$prescription might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../controllers/C_Prescription.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$files might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../custom/ajax_download.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$qrda_file_path might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/ajax_download.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/ajax_download.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/chart_tracker.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$counter might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../custom/code_types.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$query might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../custom/code_types.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sql_bind_array might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/code_types.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$cqmCodes might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/download_qrda.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../custom/download_qrda.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$facility_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/export_qrda_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$facility_name might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/export_qrda_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/export_qrda_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$stratum might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../custom/export_qrda_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$stratum_1_denom might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/export_qrda_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$stratum_1_exclude might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/export_qrda_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$stratum_1_ipp might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/export_qrda_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$stratum_1_numer1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/export_qrda_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$stratum_1_numer2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/export_qrda_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$stratum_1_numer3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/export_qrda_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$stratum_2_denom might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/export_qrda_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$stratum_2_exclude might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/export_qrda_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$stratum_2_ipp might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/export_qrda_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$stratum_2_numer1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/export_qrda_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$stratum_2_numer2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/export_qrda_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$stratum_2_numer3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/export_qrda_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userRow might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../custom/export_qrda_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../custom/export_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$alertmsg might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/import_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../custom/import_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/import_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$patients might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/qrda_category1.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$facilResRow might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../custom/qrda_category1_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$gender might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/qrda_category1_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$patFname might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/qrda_category1_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$patLname might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/qrda_category1_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$patientRow might not be defined\\.$#',
    'count' => 12,
    'path' => __DIR__ . '/../../custom/qrda_category1_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tdTitle might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../custom/qrda_category1_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$config might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../index.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/code_systems/standard_tables_manage.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/drugs/add_edit_drug.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tres might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/drugs/add_edit_drug.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$row might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/drugs/add_edit_lot.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/drugs/add_edit_lot.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/drugs/dispense_drug.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/drugs/dispense_drug.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webroot might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/drugs/dispense_drug.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/drugs/drug_inventory.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$encounter$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/eRxSOAP.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$jasonzip$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/eRx_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$prec$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/eRx_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$jasonzip might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/eRx_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$num might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/eRx_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$txt might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/eRx_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$zip4 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/eRx_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$include_root might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/easipro/pro.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/easipro/pro.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/easipro/pro.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$arr_files_php might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/expand_contract_js.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$catid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/fax/fax_dispatch.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$erow might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/fax/fax_dispatch.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$newid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/fax/fax_dispatch.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/fax/fax_dispatch.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tmp_name might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/fax/fax_dispatch.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/fax/fax_dispatch.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webserver_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/fax/fax_dispatch.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$date_init might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/LBF/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$enrow might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/LBF/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$fs might not be defined\\.$#',
    'count' => 15,
    'path' => __DIR__ . '/../../interface/forms/LBF/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ignoreAuth_onsite_portal might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/LBF/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/forms/LBF/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$svccount might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/LBF/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code_types might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/LBF/printable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/LBF/printable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pdf might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/LBF/printable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/LBF/printable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/LBF/printable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/LBF/printable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webserver_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/LBF/printable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$disabled might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/aftercare_plan/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/aftercare_plan/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/aftercare_plan/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/aftercare_plan/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/aftercare_plan/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/aftercare_plan/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/aftercare_plan/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/aftercare_plan/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/aftercare_plan/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/ankleinjury/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/ankleinjury/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/ankleinjury/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/ankleinjury/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/ankleinjury/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/ankleinjury/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/ankleinjury/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/ankleinjury/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/bronchitis/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/bronchitis/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/bronchitis/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/bronchitis/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/bronchitis/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/bronchitis/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/bronchitis/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/bronchitis/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$all might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/care_plan/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$care_plan_type might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/care_plan/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/care_plan/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/care_plan/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$data might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/care_plan/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/care_plan/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/care_plan/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/care_plan/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$key might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/care_plan/templates/careplan_actions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$key might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/care_plan/templates/careplan_reason_row.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$reasonCodeStatii might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/care_plan/templates/careplan_reason_row.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/clinic_note/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/clinic_note/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/clinic_note/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/clinic_note/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$top_bg_line might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/clinic_note/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/clinic_note/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/clinic_note/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/clinic_note/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/clinic_note/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/clinic_note/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$top_bg_line might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/clinic_note/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/clinic_note/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/clinical_instructions/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/clinical_instructions/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/clinical_instructions/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/clinical_instructions/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/clinical_instructions/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/clinical_instructions/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/clinical_notes/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/clinical_notes/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/clinical_notes/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/clinical_notes/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/dictation/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/dictation/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/dictation/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/dictation/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/dictation/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/dictation/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/dictation/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/dictation/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$BPDN might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$COMMENTS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLBRANDOD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLBRANDOS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLMANUFACTUREROD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLMANUFACTUREROS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODQUANTITY might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSQUANTITY might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTL_COMMENTS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LENS_MATERIAL might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LENS_TREATMENTS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODADD2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODAXIS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODBC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCYL might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODDIAM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODHBASE might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODHPD might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODMIDADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODMPDN might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODPDMeasured might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODSLABOFF might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODSPH might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVBASE might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVERTEXDIST might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVPD might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSADD2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSAXIS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSBC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCYL might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSDIAM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSHBASE might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSHPD might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSMIDADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSMPDN might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSPDMeasured might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSSLABOFF might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSSPH might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVBASE might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVERTEXDIST might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVPD might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$REFTYPE might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RXTYPE might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$Single might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$detailed might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$form_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$i might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$insert_this_id might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/SpectacleRx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$HELLO might not be defined\\.$#',
    'count' => 10,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ISSUE_CLASSIFICATIONS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ISSUE_TYPES might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ROSCOMMENTS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ROSCV might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ROSDERM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ROSENDOCRINE might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ROSGENERAL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ROSGI might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ROSGU might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ROSHEENT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ROSIMMUNO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ROSMUSCULO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ROSNEURO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ROSPSYCH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ROSPULM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result2 might not be defined\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$subtype might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$type_index might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/a_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/help.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ANTSEG might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/js/eye_base.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$EXT might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/js/eye_base.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$NEURO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/js/eye_base.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RETINA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/js/eye_base.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/js/eye_base.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$header1$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$k$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$old_OCTs$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$old_VFs$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT10CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT10CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT10SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT10SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT11CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT11CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT11SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT11SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT1CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT1CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT1SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT1SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT2CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT2CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT2SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT2SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT3CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT3CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT3SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT3SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT4CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT4CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT4SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT4SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT5CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT5CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT5SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT5SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT6CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT6CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT6SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT6SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT7CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT7CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT7SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT7SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT8CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT8CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT8SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT8SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT9CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT9CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT9SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT9SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ANTSEG_COMMENTS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARNEARODVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARNEAROSVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARODADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARODAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARODCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARODPRISM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARODSPH might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARODVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AROSADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AROSAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AROSCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AROSPRISM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AROSSPH might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AROSVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ATROPINE might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CACCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CACCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CONTRASTODVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRNEARODVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRNEAROSVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRODADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRODAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRODCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRODPRISM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRODSPH might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRODVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CROSADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CROSAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CROSCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CROSPRISM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CROSSPH might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CROSVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLBRANDOD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLBRANDOS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLMANUFACTUREROD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLMANUFACTUREROS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODBC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODDIAM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODSPH might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSBC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSDIAM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSSPH might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLSUPPLIEROD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLSUPPLIEROS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CYCLOGYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CYCLOMYDRIL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DACCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DACCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DIL_MEDS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DM_code might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DX might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$EXT_COMMENTS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$FAILED_drug might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$FAILED_drugs might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$GLAREODVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$GLAREOSVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$GONIO_values might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$HERTELBASE might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LADNEXA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LBROW might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LCAROTID might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LCNV might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LCNVII might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LIODVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LIOSVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LLF might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LLL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LMCT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LMRD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LTEMPART might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LUL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LVFISSURE might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITYNORMAL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LI might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LLIO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LLSO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LRIO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LRSO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RI might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RLIO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RLSO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RRIO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RRSO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRNEARODVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRNEAROSVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRODADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRODAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRODCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRODPRISM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRODSPH might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRODVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MROSADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MROSAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MROSCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MROSPRISM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MROSSPH might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MROSVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$NEO25 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$NEURO_COMMENTS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$NPC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OCT_date might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OCT_values might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODAC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODACD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODAXIALLENGTH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCMT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCOINS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCOLOR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCONJ might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCORNEA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCUP might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODDISC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODECL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODGONIO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODHERTEL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODIOP might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODIOPTARGET might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODIOPTARGETS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODIOPTARGET_values might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODIRIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODK1 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODK2 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODK2AXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODKTHICKNESS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODLENS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODLT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODMACULA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODNPA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODPDMeasured might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODPERIPH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODREDDESAT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODSCHIRMER1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODSCHIRMER2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODTBUT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVESSELS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVITREOUS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODW2W might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OD_methods might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OD_time_values might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OD_values might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSAC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSACD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSAXIALLENGTH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCMT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCOINS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCOLOR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCONJ might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCORNEA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCUP might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSDISC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSECL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSGONIO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSHERTEL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSIOPTARGET might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSIOPTARGETS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSIOPTARGET_values might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSIRIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSK1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSK2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSK2AXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSKTHICKNESS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSLENS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSLT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSMACULA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSNPA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSPDMeasured might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSPERIPH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSREDDESAT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSSCHIRMER1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSSCHIRMER2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSTBUT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVESSELS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVITREOUS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSW2W might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OS_methods might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OS_time_values might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OS_values might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$PAMODBA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$PAMODVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$PAMOSVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$PHODVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$PHOSVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RADNEXA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RBROW might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RCAROTID might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RCNV might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RCNVII might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RETINA_COMMENTS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RLF might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RLL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RMCT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RMRD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RTEMPART might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RUL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RVFISSURE might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RX_TYPE might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$STEREOPSIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$TROPICAMIDE might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$VABINOC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$VERTFUSAMPS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$VF_date might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$VF_values might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$VISITS_date might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$action might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$all might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$categories might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$children_names might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$clinical_terms might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$count might not be defined\\.$#',
    'count' => 16,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$current_OCT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$current_VF might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$date might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$display_Add might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$display_PMSFH might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$gonios might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$here might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$hideme might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$hit_RVO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$i might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$my_name might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$output might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pend might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sub_term might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$time might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$time_OU might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$total_PMSFH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$zones might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/eye_mag_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter_date might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/taskman_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$form_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/taskman_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sent_date might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/php/taskman_functions.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$id$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$table_name$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT10CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT10CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT10SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT10SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT11CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT11CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT11SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT11SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT1CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT1CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT1SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT1SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT2CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT2CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT2SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT2SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT3CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT3CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT3SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT3SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT4CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT4CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT4SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT4SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT5CCDIST might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT5CCNEAR might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT5SCDIST might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT5SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT6CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT6CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT6SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT6SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT7CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT7CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT7SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT7SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT8CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT8CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT8SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT8SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT9CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT9CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT9SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT9SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AMSLEROD might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AMSLEROS might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ANTSEG_COMMENTS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARNEARODVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARNEAROSVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARODADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARODAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARODCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARODPRISM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARODSPH might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARODVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AROSADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AROSAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AROSCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AROSPRISM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AROSSPH might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AROSVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ASSOCIATED1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ASSOCIATED2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ASSOCIATED3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CACCDIST might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CACCNEAR might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CC1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CC2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CC3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CHRONIC1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CHRONIC2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CHRONIC3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CONTEXT1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CONTEXT2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CONTEXT3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CONTRASTODVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CONTRASTOSVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRCOMMENTS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRNEARODVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRNEAROSVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRODADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRODAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRODCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRODPRISM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRODSPH might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRODVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CROSADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CROSAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CROSCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CROSPRISM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CROSSPH might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CROSVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLBRANDOD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLBRANDOS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLMANUFACTUREROD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLMANUFACTUREROS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODBC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODDIAM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODSPH might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSBC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSDIAM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSSPH might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLSUPPLIEROD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLSUPPLIEROS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DACCDIST might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DACCNEAR might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DIL_MEDS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DIMODPUPILSIZE1 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DIMODPUPILSIZE2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DIMOSPUPILSIZE1 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DIMOSPUPILSIZE2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DURATION1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DURATION2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DURATION3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$EXT_COMMENTS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$GLAREODVA might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$GLAREOSVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$HERTELBASE might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$HPI1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$HPI2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$HPI3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$IOPTIME might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LADNEXA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LBROW might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LCAROTID might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LCNV might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LCNVII might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LIODVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LIOSVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LLF might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LLL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LMCT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LMRD might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LOCATION1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LOCATION2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LOCATION3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LTEMPART might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LUL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LVFISSURE might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MODIFY1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MODIFY2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MODIFY3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITYNORMAL might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_L0 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LI might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LLIO might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LLSO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LRSO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_R0 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RI might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RLIO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RLSO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RRIO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RRSO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRNEARODVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRNEAROSVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRODADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRODAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRODCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRODPRISM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRODSPH might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRODVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MROSADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MROSAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MROSCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MROSPRISM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MROSSPH might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MROSVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$NEURO_COMMENTS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$NPC might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODAC might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODACD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODAPD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODAXIALLENGTH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCMT might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCOINS might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCOLOR might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCONJ might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCORNEA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCUP might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODDISC might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODECL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODGONIO might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODHERTEL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODIOPAP might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODIOPFTN might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODIOPTPN might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODIRIS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODK1 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODK2 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODK2AXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODKTHICKNESS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODLENS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODLT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODMACULA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODNEARVA_1 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODNPA might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODNPC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODPDMeasured might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODPERIPH might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODPUPILREACTIVITY might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODPUPILSIZE1 might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODPUPILSIZE2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODREDDESAT might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODSCHIRMER1 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODSCHIRMER2 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODTBUT might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVA_1 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVESSELS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVF might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVF1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVF2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVF3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVF4 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVITREOUS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODW2W might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSAC might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSACD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSAPD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSAXIALLENGTH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCMT might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCOINS might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCOLOR might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCONJ might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCORNEA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCUP might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSDISC might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSECL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSGONIO might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSHERTEL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSIOPAP might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSIOPFTN might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSIOPTPN might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSIRIS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSK1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSK2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSK2AXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSKTHICKNESS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSLENS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSLT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSMACULA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSNEARVA_1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSNPA might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSNPC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSPDMeasured might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSPERIPH might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSPUPILREACTIVITY might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSPUPILSIZE1 might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSPUPILSIZE2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSREDDESAT might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSSCHIRMER1 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSSCHIRMER2 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSTBUT might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVA_1 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVESSELS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVF might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVF1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVF2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVF3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVF4 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVITREOUS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSW2W might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$PAMODVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$PAMOSVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$PHODVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$PHOSVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$PUPIL_COMMENTS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$PUPIL_NORMAL might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$QUALITY1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$QUALITY2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$QUALITY3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RADNEXA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RBROW might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RCAROTID might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RCNV might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RCNVII might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RETINA_COMMENTS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RLF might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RLL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RMCT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RMRD might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RTEMPART might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RUL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RVFISSURE might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RX_TYPE might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$SCNEARODVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$SCNEAROSVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$SCODVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$SCOSVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$SEVERITY1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$SEVERITY2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$SEVERITY3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$STEREOPSIS might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$TIMING1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$TIMING2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$TIMING3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$VERTFUSAMPS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$display_Add might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter_date might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pend might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DOCS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$N might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$PMSFH might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$category_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$dups might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$form_encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$form_injury_part might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$form_injury_type might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ndc_info might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$new_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$printable might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$text_type might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$visit_date might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webroot might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webserver_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/taskman.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT10CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT10CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT10SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT10SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT11CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT11CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT11SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT11SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT1CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT1CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT1SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT1SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT2CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT2CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT2SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT2SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT3CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT3CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT3SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT3SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT4CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT4CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT4SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT4SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT5CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT5CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT5SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT5SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT6CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT6CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT6SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT6SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT7CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT7CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT7SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT7SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT8CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT8CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT8SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT8SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT9CCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT9CCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT9SCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ACT9SCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AMSLEROD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AMSLEROS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ANTSEG_COMMENTS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARNEARODVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARNEAROSVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARODADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARODAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARODCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARODPRISM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARODSPH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ARODVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AROSADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AROSAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AROSCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AROSPRISM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AROSSPH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AROSVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ASSOCIATED1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ASSOCIATED2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ASSOCIATED3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ATROPINE might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$BALANCED might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$BINOCVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CACCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CACCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CC1 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CC2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CC3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CHRONIC1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CHRONIC2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CHRONIC3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CONTEXT1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CONTEXT2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CONTEXT3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRCOMMENTS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRODAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRODCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRODSPH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CRODVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CROSAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CROSCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CROSSPH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CROSVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLBRANDOD might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLBRANDOS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLBRAND_list_OD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLBRAND_list_OS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLMANUFACTUREROD might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLMANUFACTUREROS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODBC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODDIAM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODSPH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLODVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSBC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSDIAM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSSPH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLOSVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLSUPPLIEROD might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTLSUPPLIEROS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CTL_COMMENTS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CYCLOGYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$CYCLOMYDRIL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DACCDIST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DACCNEAR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DIL_MEDS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DIL_RISKS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DIMODPUPILSIZE1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DIMODPUPILSIZE2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DIMOSPUPILSIZE1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DIMOSPUPILSIZE2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DURATION1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DURATION2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$DURATION3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$EXT_COMMENTS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$GLAREODVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$GLAREOSVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$HERTELBASE might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$HPI1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$HPI2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$HPI3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$IMP might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$IOPTIME might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LADNEXA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LBROW might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LCAROTID might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LCNV might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LCNVII might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LIODVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LIOSVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LLF might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LLL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LMCT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LMRD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LOCATION1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LOCATION2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LOCATION3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LOCKED might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LOCKEDBY might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LTEMPART might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LUL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$LVFISSURE might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MODIFY1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MODIFY2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MODIFY3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITYNORMAL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LI might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LL might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LLIO might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LLSO might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LR might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LRIO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LRSO might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_LS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RI might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RL might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RLIO might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RLSO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RR might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RRIO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RRSO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MOTILITY_RS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRNEARODVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRNEAROSVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRODADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRODAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRODCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRODPRISM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRODSPH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MRODVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MROSADD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MROSAXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MROSCYL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MROSPRISM might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MROSSPH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MROSVA might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$NEO25 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$NEURO_COMMENTS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$NPC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODAC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODACD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODAPD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODAXIALLENGTH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCMT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCOINS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCOLOR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCONJ might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCORNEA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODCUP might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODDISC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODGONIO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODHERTEL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODIOPAP might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODIOPFTN might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODIOPPOST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODIOPTPN might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODIRIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODK1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODK2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODK2AXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODKTHICKNESS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODLENS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODLT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODMACULA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODNPA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODPDMeasured might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODPERIPH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODPUPILREACTIVITY might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODPUPILSIZE1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODPUPILSIZE2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODREDDESAT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODSCHIRMER1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODSCHIRMER2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODTBUT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVESSELS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVF might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVF1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVF2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVF3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVF4 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODVITREOUS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ODW2W might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSAC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSACD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSAPD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSAXIALLENGTH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCMT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCOINS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCOLOR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCONJ might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCORNEA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSCUP might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSDISC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSGONIO might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSHERTEL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSIOPAP might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSIOPFTN might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSIOPPOST might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSIOPTPN might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSIRIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSK1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSK2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSK2AXIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSKTHICKNESS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSLENS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSLT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSMACULA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSNPA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSPDMeasured might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSPERIPH might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSPUPILREACTIVITY might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSPUPILSIZE1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSPUPILSIZE2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSREDDESAT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSSCHIRMER1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSSCHIRMER2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSTBUT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVESSELS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVF might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVF1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVF2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVF3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVF4 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSVITREOUS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OSW2W might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$PAMODVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$PAMOSVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$PHODVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$PHOSVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$PUPIL_COMMENTS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$PUPIL_NORMAL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$QUALITY1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$QUALITY2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$QUALITY3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RADNEXA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RBROW might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RCAROTID might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RCNV might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RCNVII might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RETINA_COMMENTS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RLF might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RLL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RMCT might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RMRD might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RTEMPART might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RUL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RVFISSURE might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$SCNEARODVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$SCNEAROSVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$SCODVA might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$SCOSVA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$SEVERITY1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$SEVERITY2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$SEVERITY3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$STEREOPSIS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$TIMING1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$TIMING2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$TIMING3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$TROPICAMIDE might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$VERTFUSAMPS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$WETTYPE might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$alert might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$confused might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter_date might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$oriented might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$provider_id might not be defined\\.$#',
    'count' => 12,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_js_includes might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/eye_mag/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/code_choice/initialize_code_choice.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_js_includes might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/contraception_products/initialize_contraception_products.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/contraception_products/initialize_contraception_products.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$cleave_opt might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code_types might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ndc_applies might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$entry might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/code_check.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$database might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/fee_sheet_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$issues might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/fee_sheet_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$req_encounter might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/fee_sheet_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$req_pid might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/fee_sheet_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$task might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/fee_sheet_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$billing_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/fee_sheet_justify.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$database might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/fee_sheet_justify.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$json_diags might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/fee_sheet_justify.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$req_encounter might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/fee_sheet_justify.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$req_pid might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/fee_sheet_justify.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$task might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/fee_sheet_justify.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/fee_sheet_queries.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/fee_sheet_search_queries.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/initialize_review.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$isBilled might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/initialize_review.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/initialize_review.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/initialize_review.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/fee_sheet/review/views/review.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/functional_cognitive_status/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/functional_cognitive_status/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/functional_cognitive_status/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/functional_cognitive_status/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/functional_cognitive_status/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/gad7/gad7.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/gad7/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_default might not be defined\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../interface/forms/gad7/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_extremely might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_form_name might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_form_title might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_more might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/forms/gad7/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_nearly might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/forms/gad7/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_nosave_confirm might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_nosave_exit might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/gad7/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_not might not be defined\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../interface/forms/gad7/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_q8 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_q8_2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_several might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/forms/gad7/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_somewhat might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_very might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$data might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/gad7/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/gad7/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_afraid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_annoyed might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_control_worry might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_default might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_extremely might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_form_name might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_form_title might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_more might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_nearly might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_nervous might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_nosave_confirm might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_nosave_exit might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_not might not be defined\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_q8 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_q8_2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_relax might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_restless might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_several might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_somewhat might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_very might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_worry might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/gad7/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/group_attendance/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/group_attendance/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$therapy_group might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/group_attendance/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/group_attendance/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$therapy_group might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/group_attendance/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/group_attendance/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/misc_billing_options/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/misc_billing_options/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/misc_billing_options/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/forms/misc_billing_options/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/misc_billing_options/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/misc_billing_options/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/misc_billing_options/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/misc_billing_options/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webroot might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/misc_billing_options/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$disabled might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/newGroupEncounter/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/newGroupEncounter/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$posCode might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/newGroupEncounter/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result might not be defined\\.$#',
    'count' => 14,
    'path' => __DIR__ . '/../../interface/forms/newGroupEncounter/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/newGroupEncounter/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/newGroupEncounter/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$therapyGroupCategories might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/newGroupEncounter/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$therapy_group might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/newGroupEncounter/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$viewmode might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/newGroupEncounter/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/newGroupEncounter/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/newGroupEncounter/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/newGroupEncounter/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/newGroupEncounter/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/newGroupEncounter/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/newpatient/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/newpatient/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/newpatient/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/newpatient/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/newpatient/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/newpatient/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/newpatient/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/newpatient/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/newpatient/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/note/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/note/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$obj might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/forms/note/print.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/note/print.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/note/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/note/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/note/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/note/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$obj might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/forms/note/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/note/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/note/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/phq9/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/phq9/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/phq9/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/phq9/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/phq9/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$i might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/physical_exam/edit_diagnoses.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$issue might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/physical_exam/edit_diagnoses.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/physical_exam/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pelines might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/physical_exam/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/physical_exam/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/physical_exam/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/physical_exam/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/physical_exam/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/prior_auth/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/prior_auth/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/prior_auth/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/procedure_order/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$isDorn might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/procedure_order/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 12,
    'path' => __DIR__ . '/../../interface/forms/procedure_order/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/procedure_order/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/procedure_order/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/procedure_order/common.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/procedure_order/delete.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/procedure_order/delete.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/procedure_order/delete.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$i might not be defined\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../interface/forms/procedure_order/templates/procedure_reason_row.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$reasonCodeStatii might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/procedure_order/templates/procedure_reason_row.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$do_warning might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/questionnaire_assessments/questionnaire_assessments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$mode might not be defined\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../interface/forms/questionnaire_assessments/questionnaire_assessments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/questionnaire_assessments/questionnaire_assessments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/questionnaire_assessments/questionnaire_assessments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/questionnaire_assessments/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/questionnaire_assessments/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/questionnaire_assessments/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/questionnaire_assessments/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/requisition/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/reviewofs/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/reviewofs/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/reviewofs/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/reviewofs/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/reviewofs/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/reviewofs/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/reviewofs/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/reviewofs/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/ros/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/ros/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/ros/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/sdoh/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/sdoh/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/sdoh/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$formid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/sdoh/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/sdoh/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/sdoh/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/sdoh/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/soap/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/soap/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/soap/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/track_anything/create.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/track_anything/create.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/forms/track_anything/create.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/track_anything/create.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/forms/track_anything/create.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$fromencounter might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/track_anything/history.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/track_anything/history.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$row might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/track_anything/history.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/track_anything/history.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/track_anything/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/track_anything/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/track_anything/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/track_anything/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/track_anything/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/track_anything/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$showbutton might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/track_anything/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/transfer_summary/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/transfer_summary/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/transfer_summary/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/transfer_summary/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/transfer_summary/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/transfer_summary/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/transfer_summary/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/transfer_summary/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$disabled might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/treatment_plan/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/treatment_plan/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/treatment_plan/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/treatment_plan/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/treatment_plan/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/treatment_plan/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/treatment_plan/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/treatment_plan/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms/treatment_plan/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$results might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/vitals/C_FormVitals.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$HC_delta_x might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$HC_delta_y might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$HC_dot_x might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$HC_dot_y might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$HT_delta_x might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$HT_x might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$HToffset might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$WT_delta_y might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$WT_y might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$WToffset might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$age might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$age_in_months might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ageinYMD might not be defined\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$bmi might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$bmi_delta_x might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$bmi_delta_y might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$bmi_dot_x might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$bmi_dot_y might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$chart might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$chartCss1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$chartCss2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$datatable2_bmi_offset might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$datatable2_hc_offset might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$datatable_bmi_offset might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$datatable_hc_offset might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$dob might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/forms/vitals/growthchart/chart.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/vitals/new.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$bps might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/vitals/report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/vitals/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/forms/vitals/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$inDir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/forms_admin/forms_admin.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/forms_admin/forms_admin.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$portal_temp_css_theme_name might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/globals.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$temp_css_theme_name might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/globals.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_js_includes might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../interface/globals.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webroot might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/help_modal.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$create might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/language/csv/translation_utilities.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$go might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/language/lang_definition.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/language/language.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$openemr_name might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/login/login.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/login_screen.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$getevent might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/logview/logview.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$openemr_name might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/about_page.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$authorize might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/main/authorizations/authorizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$groupname might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/main/authorizations/authorizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/authorizations/authorizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/authorizations/authorizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/authorizations/authorizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result4 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/authorizations/authorizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/authorizations/authorizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/main/authorizations/authorizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tmore might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/authorizations/authorizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$authorize might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/main/authorizations/authorizations_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$groupname might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/main/authorizations/authorizations_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/authorizations/authorizations_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/authorizations/authorizations_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tback might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/authorizations/authorizations_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$UOR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/backup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$datatypes might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/backup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sources might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/backup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/backup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webserver_root might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/main/backup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$zipname might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/backup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sqlconf might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/main/backuplog.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$duration might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/main/calendar/add_edit_event.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$e2f might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/add_edit_event.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$endtime might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/main/calendar/add_edit_event.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$event_date might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/main/calendar/add_edit_event.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$locationspec might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/main/calendar/add_edit_event.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$multiple_value might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/calendar/add_edit_event.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$my_recurrtype might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/main/calendar/add_edit_event.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$new_eid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/add_edit_event.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$noRecurrspec might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/calendar/add_edit_event.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$providers_current might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/main/calendar/add_edit_event.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$recurrence_end_date might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/add_edit_event.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$recurrspec might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/main/calendar/add_edit_event.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$starttime might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/main/calendar/add_edit_event.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$isProv might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/calendar/find_appt_popup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$providerid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/find_appt_popup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$slots might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/find_appt_popup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/find_appt_popup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/find_group_popup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/find_group_popup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/find_patient_popup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$_GLOBALS might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/calendar/includes/pnAPI.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/calendar/index.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pcDir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/common.api.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$this might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/plugins/function.pc_date_format.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$catoptions might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/plugins/function.pc_filter.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$class might not be defined\\.$#',
    'count' => 11,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/plugins/function.pc_filter.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$useroptions might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/plugins/function.pc_filter.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$function might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/plugins/function.pc_sort_events.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$order might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/plugins/function.pc_sort_events.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sort might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/plugins/function.pc_sort_events.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$value might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/plugins/function.pc_sort_events.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$link might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/plugins/function.pc_url.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$localpath might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/plugins/function.pc_url.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$setdeftime might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/plugins/function.pc_url.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$t might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/plugins/function.pc_view_select.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$smarty might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/plugins/modifier.pc_date_format.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$msg$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnadmin.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$dels might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnadmin.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$aco might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnadminapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$active might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnadminapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$color might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnadminapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$constantid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnadminapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$desc might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnadminapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$duration might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnadminapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$end_all_day might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnadminapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$end_date_flag might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnadminapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$end_date_freq might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnadminapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$end_date_type might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnadminapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$limitid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnadminapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$recurrfreq might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnadminapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$repeat might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnadminapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sequence might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnadminapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$spec might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnadminapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$value_cat_type might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnadminapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$months might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnincludes/Date/Calc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$weekdays might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnincludes/Date/Calc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$cDay might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pntemplates/default/views/monthSelector.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$day$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnuser.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$month$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnuser.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$sqlKeywords$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnuser.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$cacheid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnuserapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ending_date might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnuserapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$etime might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnuserapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$event_status might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnuserapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$nuke_users might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnuserapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$patient_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnuserapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$provider_id might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnuserapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$starting_date might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnuserapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$stime might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnuserapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userid might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnuserapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$viewtype might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/calendar/modules/PostCalendar/pnuserapi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pnconfig might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/calendar/pntables.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/dated_reminders/dated_reminders.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ReminderSent might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/dated_reminders/dated_reminders_add.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/dated_reminders/dated_reminders_add.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webroot might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/dated_reminders/dated_reminders_add.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/display_documents.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/finder/dynamic_finder.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$aColumns might not be defined\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../interface/main/finder/dynamic_finder_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$results might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/finder/multi_patients_finder.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/finder/multi_patients_finder.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$results might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/finder/multi_patients_finder_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/finder/multi_patients_finder_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$itemized_test_id might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/finder/patient_select.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$numerator_label might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/finder/patient_select.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pass_id might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/finder/patient_select.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$report_id might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/main/finder/patient_select.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/main/finder/patient_select.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/ippf_export.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/main_screen.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$include_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/messages/lab_results_messages.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/messages/lab_results_messages.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$where might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/messages/lab_results_messages.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$background might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/messages/messages.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$body might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/messages/messages.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$cursor might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/messages/messages.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$message_legend might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/messages/messages.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/main/messages/messages.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sortlink might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/main/messages/messages.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/main/messages/messages.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/messages/messages.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_js_includes might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/messages/messages.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webroot might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/messages/messages.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webserver_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/messages/messages.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$prov_name might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/messages/print_postcards.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OE_SITE_DIR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/messages/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$facilities might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/messages/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$providers might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/messages/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$results might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/messages/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/main/messages/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/messages/save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$row might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/main/messages/templates/linked_documents.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$documentId might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/messages/trusted-messages-ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$message might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/messages/trusted-messages-ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/main/messages/trusted-messages-ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$recipient might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/main/messages/trusted-messages-ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$requested_by might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/main/messages/trusted-messages-ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_js_includes might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/messages/trusted-messages.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webroot might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/messages/trusted-messages.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tmore might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/onotes/office_comments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$any_search_class might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/tabs/main.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$dispatcher might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/tabs/main.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$fileroot might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/tabs/main.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$openemr_name might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/main/tabs/main.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$search_any_type might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/tabs/main.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$search_globals_class might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/tabs/main.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_js_includes might not be defined\\.$#',
    'count' => 9,
    'path' => __DIR__ . '/../../interface/main/tabs/main.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/main/tabs/main.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webroot might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/main/tabs/main.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$classLoader might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/openemr.bootstrap.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$eventDispatcher might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/openemr.bootstrap.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$benefit might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/additional_info.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$benefits might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/benefit.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$coverageResults might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/coverage_discovery_results.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$benefit might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/date_information.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$eligibilityData might not be defined\\.$#',
    'count' => 20,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/deductibles.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$demographicInfo might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/demographics_results.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/eligibility.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$chatPayerCode might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/eligibility_chat.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$chatPrKey might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/eligibility_chat.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$chatProductResultIds might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/eligibility_chat.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$benefit might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/identifier_info.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$mbiResults might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/mbi_results.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$eligibilityData might not be defined\\.$#',
    'count' => 22,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/medicare_info.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$benefit might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/messages.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tab might not be defined\\.$#',
    'count' => 15,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/navbar.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$benefit might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/procedure_info.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$receiver might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/receiver.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$benefit might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/related_entity.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$benefit might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/service_delivery.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$source might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/source.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$subscriberPatient might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/subscriber_patient.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$data might not be defined\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/validation.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$eligibilityData might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/templates/validation.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$classLoader might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-comlink-telehealth/openemr.bootstrap.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$eventDispatcher might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-comlink-telehealth/openemr.bootstrap.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$user might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-comlink-telehealth/src/Controller/TeleHealthVideoRegistrationController.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userSaveRecord might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-comlink-telehealth/src/Services/TeleHealthRemoteRegistrationService.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$classLoader might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/openemr.bootstrap.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$eventDispatcher might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/openemr.bootstrap.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$datas might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/public/lab_setup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$data might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/public/primary_config_edit.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$npi might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/public/primary_config_edit.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/src/AddressBookAddEdit.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$D might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/src/DornGenHl7Order.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$segment might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/src/DornGenHl7Order.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$segment might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/src/GenHl7OrderBase.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$arep$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/src/ReceiveHl7Results.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$log$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/src/ReceiveHl7Results.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$prpath$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/src/ReceiveHl7Results.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tab might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/templates/navbar.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$classLoader might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-ehi-exporter/openemr.bootstrap.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$eventDispatcher might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-ehi-exporter/openemr.bootstrap.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$taskId might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-ehi-exporter/public/index.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$job might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-ehi-exporter/src/Services/EhiExporter.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-faxsms/contact.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$selectedService might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-faxsms/library/setup_services.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-faxsms/library/utility.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-faxsms/library/utility.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$classLoader might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-faxsms/openemr.bootstrap.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-faxsms/setup_rc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-faxsms/setup_voice.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pname might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-faxsms/src/Controller/RCFaxClient.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$response might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-faxsms/src/Controller/RCFaxClient.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$message might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-faxsms/src/Events/NotificationEventListener.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$classLoader might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/openemr.bootstrap.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$eventDispatcher might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/openemr.bootstrap.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$is_saved might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/src/Services/LogImportBuild.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/templates/indexrx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/templates/indexrx.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$usersData might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/templates/weno_users.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$array_active_modules might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Acl/src/Acl/Controller/AclController.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rows might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Application/src/Application/Plugin/CommonPlugin.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$his_tob_date might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Controller/CarecoordinationController.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$combination$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Controller/EncounterccdadispatchController.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Controller/EncounterccdadispatchController.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$combination might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Controller/EncountermanagerController.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$allergy_begdate_value might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Model/CarecoordinationTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$o_id might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Model/CarecoordinationTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$provider_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Model/CarecoordinationTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$value might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Model/CarecoordinationTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Model/CcdaGenerator.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Model/CcdaGenerator.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$count might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Model/EncounterccdadispatchTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter_diagnosis might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Model/EncounterccdadispatchTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$formatted_date might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Model/EncounterccdadispatchTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$forms might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Model/EncounterccdadispatchTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$query might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Model/EncounterccdadispatchTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$row_patient_data might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Model/EncounterccdadispatchTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$start_date might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Model/EncounterccdadispatchTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$trans_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Model/EncounterccdadispatchTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ccda_id might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Model/EncountermanagerTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$elec_sent might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Model/EncountermanagerTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$formatted_date might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Model/EncountermanagerTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$trans_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Carecoordination/src/Carecoordination/Model/EncountermanagerTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$active might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Ccr/src/Ccr/Model/CcrTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$activity might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Ccr/src/Ccr/Model/CcrTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rows might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Immunization/src/Immunization/Controller/ImmunizationController.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$curr_html_tag might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Installer/src/Installer/Controller/InstallerController.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$status might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Installer/src/Installer/Controller/InstallerController.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_major might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Installer/src/Installer/Controller/InstallerController.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_minor might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Installer/src/Installer/Controller/InstallerController.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_patch might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Installer/src/Installer/Controller/InstallerController.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ret_str might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Installer/src/Installer/Model/InstModuleTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$aclSetupFlag might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/PatientFilter/acl/acl_setup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$aclSetupFlag might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/PatientFilter/acl/acl_upgrade.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$drug_test_count might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/PatientFlowBoard/src/PatientFlowBoard/Listener/PatientFlowBoardEventsSubscriber.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$p might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/PrescriptionTemplates/src/PrescriptionTemplates/Controller/PrescriptionTemplatesController.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$formatted_date might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Syndromicsurveillance/src/Syndromicsurveillance/Model/SyndromicsurveillanceTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$text might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/zend_modules/module/Syndromicsurveillance/src/Syndromicsurveillance/Model/SyndromicsurveillanceTable.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$BS_COL_CLASS might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/new/new_comprehensive.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$date_init might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/new/new_comprehensive.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/new/new_comprehensive.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/new/new_comprehensive_save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/new/new_comprehensive_save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$alertmsg might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/new/new_patient_save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/new/new_patient_save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/new/new_patient_save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/new/new_patient_save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/new/new_search_popup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/addr_label.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/barcode_label.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$type might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/barcode_label.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OE_SITE_DIR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/download_template.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/download_template.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/patient_file/download_template.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OE_SITE_DIR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/education.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/education.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$counter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/cash_receipt.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/cash_receipt.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/cash_receipt.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$raw_encounter_date might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/cash_receipt.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/cash_receipt.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code_types might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/coding.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/coding.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$all might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/copay.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/copay.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/delete_form.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$incdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/delete_form.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/delete_form.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/delete_form.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$counter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/diagnosis.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/diagnosis.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/diagnosis.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/diagnosis.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tback might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/diagnosis.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tmore might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/diagnosis.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/diagnosis.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/diagnosis_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$counter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/diagnosis_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/diagnosis_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/diagnosis_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tback might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/diagnosis_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$text might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/diagnosis_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$type might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/diagnosis_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/diagnosis_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/encounter_bottom.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/encounter_top.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/encounter_top.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/encounter_top.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code_types might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/find_code_dynamic.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code_external_tables might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/find_code_dynamic_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code_types might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/find_code_dynamic_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$codetype might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/find_code_dynamic_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$fe_array might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/find_code_dynamic_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$from might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/find_code_dynamic_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$include_inactive might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/find_code_dynamic_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sellist might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/find_code_dynamic_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$source might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/find_code_dynamic_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/find_code_dynamic_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$where1 might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/find_code_dynamic_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/find_code_history.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code_types might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/find_code_popup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$groupId might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/forms.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code_types might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/search_code.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/search_code.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/search_code.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code_types might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/select_codes.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code_types might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/superbill_codes.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/superbill_codes.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/superbill_codes.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tback might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/superbill_codes.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tmore might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/superbill_codes.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/superbill_codes.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/superbill_custom_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code_type might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/superbill_custom_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code_type_name_external might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/superbill_custom_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code_types might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/superbill_custom_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$filter_key might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/superbill_custom_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$modifier might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/superbill_custom_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$res might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/superbill_custom_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/superbill_custom_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$taxrates might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/superbill_custom_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$incdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/encounter/trend_form.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$cryptoGen might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/front_payment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/patient_file/front_payment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$form_pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/front_payment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$payment_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/front_payment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/front_payment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$session_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/front_payment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/patient_file/front_payment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ccaudit might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/front_payment_cc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/front_payment_cc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$response might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/front_payment_cc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ISSUE_TYPES might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/history/encounters.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$attendant_type might not be defined\\.$#',
    'count' => 13,
    'path' => __DIR__ . '/../../interface/patient_file/history/encounters.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$dres might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/history/encounters.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter_type might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/history/encounters.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 22,
    'path' => __DIR__ . '/../../interface/patient_file/history/encounters.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/patient_file/history/encounters.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$therapy_group might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/patient_file/history/encounters.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/history/encounters_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$include_root might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/history/history.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$list_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/history/history.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/patient_file/history/history.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/history/history.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$date_init might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/history/history_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$include_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/history/history_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/patient_file/history/history_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/patient_file/history/history_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/history/history_save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/history/history_save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$last might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/label.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pdf might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/label.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/label.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/letter.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/manage_dup_patients.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OE_SITE_DIR might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/merge_patients.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$source_row might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/merge_patients.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/merge_patients.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tables might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/merge_patients.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$contraception_billing_code might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/pos_checkout_ippf.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$form_num_amount_columns might not be defined\\.$#',
    'count' => 10,
    'path' => __DIR__ . '/../../interface/patient_file/pos_checkout_ippf.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$form_num_method_columns might not be defined\\.$#',
    'count' => 9,
    'path' => __DIR__ . '/../../interface/patient_file/pos_checkout_ippf.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$form_num_ref_columns might not be defined\\.$#',
    'count' => 9,
    'path' => __DIR__ . '/../../interface/patient_file/pos_checkout_ippf.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$form_num_type_columns might not be defined\\.$#',
    'count' => 9,
    'path' => __DIR__ . '/../../interface/patient_file/pos_checkout_ippf.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$num_optional_columns might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/pos_checkout_ippf.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/pos_checkout_ippf.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/patient_file/pos_checkout_ippf.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$thisdate might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/pos_checkout_ippf.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$amount might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/pos_checkout_normal.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/pos_checkout_normal.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/pos_checkout_normal.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/pos_checkout_normal.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/printed_fee_sheet.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pages might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/printed_fee_sheet.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$patdata might not be defined\\.$#',
    'count' => 9,
    'path' => __DIR__ . '/../../interface/patient_file/printed_fee_sheet.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/printed_fee_sheet.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/printed_fee_sheet.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/printed_fee_sheet.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webserver_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/printed_fee_sheet.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ISSUE_TYPES might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/problem_encounter.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/problem_encounter.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webroot might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/problem_encounter.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/reminder/active_reminder_popup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/reminder/active_reminder_popup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/reminder/clinical_reminders.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/reminder/clinical_reminders.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rules_default might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/reminder/patient_reminders.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sortlink might not be defined\\.$#',
    'count' => 11,
    'path' => __DIR__ . '/../../interface/patient_file/reminder/patient_reminders.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/patient_file/reminder/patient_reminders.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ISSUE_TYPES might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/report/custom_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OE_SITE_DIR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/report/custom_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$insurance_data_array might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/report/custom_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 22,
    'path' => __DIR__ . '/../../interface/patient_file/report/custom_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$prevIssueType might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/report/custom_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/patient_file/report/custom_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tback might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/report/custom_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tmp_files_remove might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/report/custom_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_js_includes might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/report/custom_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ISSUE_TYPES might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/report/patient_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$include_root might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/report/patient_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../interface/patient_file/report/patient_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/report/patient_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/report/patient_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/rules/patient_data.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/rules/patient_data.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$amendment_desc might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/add_edit_amendments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/add_edit_amendments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$resultSet might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/add_edit_amendments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/add_edit_amendments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ISSUE_CLASSIFICATIONS might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/add_edit_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ISSUE_TYPES might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../interface/patient_file/summary/add_edit_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/add_edit_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_js_includes might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/add_edit_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webroot might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/add_edit_issue.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$form_adreviewed might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/advancedirectives.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$form_completedad might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/advancedirectives.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/advancedirectives.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/advancedirectives.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/browse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tback might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/browse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/clinical_reminders_fragment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/clinical_reminders_fragment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/patient_file/summary/create_portallogin.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$oemr_ui might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/dashboard_header.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/dashboard_header.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/dashboard_header.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$expand_icon_class might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/dashboard_header_simple_return.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$expand_title might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/dashboard_header_simple_return.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$expandable might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/dashboard_header_simple_return.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$go_back_href might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/dashboard_header_simple_return.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$header_title might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/dashboard_header_simple_return.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$help_icon might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/dashboard_header_simple_return.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/dashboard_header_simple_return.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$bgColor might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/demographics.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/summary/demographics.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$firstApptIndx might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/demographics.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$include_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/demographics.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$num_of_days might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/demographics.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$query might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/demographics.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$resNotNull might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/summary/demographics.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../interface/patient_file/summary/demographics.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$vitals_form_id might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/summary/demographics.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$condition_str might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/demographics_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$date_init might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/demographics_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../interface/patient_file/summary/demographics_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/summary/demographics_print.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/demographics_print.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/demographics_print.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webserver_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/demographics_print.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/demographics_save.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/disc_fragment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$active might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/disclosure_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/patient_file/summary/disclosure_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/disclosure_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$administered_by might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/immunizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/immunizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code_text might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/immunizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code_type might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/immunizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$id might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/summary/immunizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$immunization_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/immunizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$imo_criteria_value might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/immunizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$observation_criteria might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/immunizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$observation_criteria_value might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/immunizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/immunizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$vis_presented_dateval might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/immunizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$vis_published_dateval might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/immunizations.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/insurance_edit.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/patient_file/summary/insurance_edit.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$date_collected might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/labdata.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$nothing might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/labdata.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/patient_file/summary/labdata.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result_code might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/labdata.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/labdata.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$the_item might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/labdata.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/labdata.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/labdata_fragment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/lbf_fragment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/list_amendments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/list_amendments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/patient_reminders_fragment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/patient_reminders_fragment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/pnotes.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/summary/pnotes.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tmore might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/pnotes.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/patient_file/summary/pnotes_fragment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/summary/pnotes_fragment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$notes_sent_count might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/pnotes_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/pnotes_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result_count might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/pnotes_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result_sent_count might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/pnotes_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/pnotes_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/pnotes_full_add.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/patient_file/summary/pnotes_full_add.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/pnotes_full_add.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/pnotes_print.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/summary/pnotes_print.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/print_amendments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/print_amendments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/record_disclosure.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$title$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/shot_record.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$data might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/shot_record.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/shot_record.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/shot_record.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ISSUE_TYPES might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/stats.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$kernel might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/stats.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$need_head might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/stats.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../interface/patient_file/summary/stats.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/stats.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ISSUE_TYPES might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/stats_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$include_root might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/summary/stats_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/patient_file/summary/stats_full.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/track_anything_fragment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/summary/vitals_fragment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$date_init might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/transaction/add_transaction.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$include_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/transaction/add_transaction.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 12,
    'path' => __DIR__ . '/../../interface/patient_file/transaction/add_transaction.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/patient_file/transaction/add_transaction.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/transaction/add_transaction.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_file/transaction/print_referral.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/transaction/print_referral.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webserver_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/transaction/print_referral.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/transaction/record_request.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$include_root might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/transaction/transactions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/transaction/transactions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_file/transaction/transactions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_file/void_dialog.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MedEx might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_tracker/patient_tracker.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$count_facs might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/patient_tracker/patient_tracker.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$count_provs might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_tracker/patient_tracker.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$current_events might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_tracker/patient_tracker.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$from_date might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/patient_tracker/patient_tracker.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$icon2_here might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_tracker/patient_tracker.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$icon_4_CALL might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_tracker/patient_tracker.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$icon_here might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_tracker/patient_tracker.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$local might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_tracker/patient_tracker.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ptkr_future_time might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_tracker/patient_tracker.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$select_facs might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_tracker/patient_tracker.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$select_provs might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_tracker/patient_tracker.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../interface/patient_tracker/patient_tracker.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$statuses_list might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_tracker/patient_tracker.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_js_includes might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_tracker/patient_tracker.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webserver_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/patient_tracker/patient_tracker.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/patient_tracker/patient_tracker_status.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/procedure_tools/ereqs/ereq_universal_form.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/procedure_tools/ereqs/ereq_universal_form.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webserver_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/procedure_tools/gen_universal_hl7/gen_hl7_order.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/procedure_tools/labcorp/ereq_form.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/procedure_tools/labcorp/ereq_form.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webserver_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/procedure_tools/labcorp/gen_hl7_order.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webserver_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/procedure_tools/quest/gen_hl7_order.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/amc_full_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/amc_tracking.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rems_out might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/appointments_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/reports/appointments_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code_types might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/reports/appt_encounter_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$res might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/appt_encounter_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/appt_encounter_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$checkSumNewApi might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/audit_log_tamper_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$getevent might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/audit_log_tamper_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/cdr_log.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/chart_location_activity.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/chart_location_activity.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/charts_checked_out.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$mh_stmt might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/clinical_reports.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/clinical_reports.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ins_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/collections_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$insposition might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/collections_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$invlines might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/collections_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$provider_name might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/collections_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/collections_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/reports/cqm.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$EXPORT_INC might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/criteria.tab.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$RadioSeperator might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/criteria.tab.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$TPSCriteriaDataTypeMaster might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/criteria.tab.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$TPSCriteriaDisplayMaster might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/criteria.tab.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$TPSCriteriaDisplayRadioMaster might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/criteria.tab.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$TPSCriteriaIncludeMaster might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/criteria.tab.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$TPSCriteriaKeyMaster might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/criteria.tab.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$TPSCriteriaQueryDropDownMaster might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/criteria.tab.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$TPSCriteriaQueryDropDownMasterDefault might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/criteria.tab.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$TPSCriteriaQueryDropDownMasterDefaultKey might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/criteria.tab.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$TPSCriteriaRadioKeyMaster might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/criteria.tab.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$TextSeperator might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/criteria.tab.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$daysheet might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/criteria.tab.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$daysheet_total might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/criteria.tab.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$provider_run might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/criteria.tab.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webroot might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/criteria.tab.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webserver_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/criteria.tab.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$insurance_data_array might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/reports/custom_report_range.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$patient_data_array might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/custom_report_range.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pids might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../interface/reports/custom_report_range.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/reports/custom_report_range.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/daily_summary_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/destroyed_drugs_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/reports/encounters_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$include_root might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/external_data.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/external_data.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/external_data.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/front_receipts_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$total1_by_method might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/reports/front_receipts_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$total2_by_method might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/reports/front_receipts_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/immunization_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/inventory_activity.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$include_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/inventory_list.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/inventory_list.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/inventory_transactions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$productleft might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/ippf_cyp_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/ippf_cyp_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$report_type might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/ippf_daily.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$form_report might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/ippf_statistics.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/message_list.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$openemr_name might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/non_reported.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/non_reported.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$effectiveInsurances might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/pat_ledger.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$include_root might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/pat_ledger.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$last_year might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/pat_ledger.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/pat_ledger.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/reports/patient_flow_board_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/patient_list.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$odrstmt might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/patient_list_creation.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sortlink might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/patient_list_creation.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/patient_list_creation.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/prescriptions_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$invnumber$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/receipts_by_method_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rowmethod might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/receipts_by_method_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/receipts_by_method_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/referrals_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/reports/report_results.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/reports/sales_by_item.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code_types might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/reports/services_by_category.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$key might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/services_by_category.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/reports/svc_code_financial_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/reports/unique_seen_patients_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$logocode might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/smart/register-app.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tinylogocode1 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/smart/register-app.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tinylogocode2 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/smart/register-app.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$statuses might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/therapy_groups/therapy_groups_views/addGroup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$users might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/therapy_groups/therapy_groups_views/addGroup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$events might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/therapy_groups/therapy_groups_views/appointmentComponent.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$groupId might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/therapy_groups/therapy_groups_views/groupDetailsGeneralData.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$users might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/therapy_groups/therapy_groups_views/groupDetailsGeneralData.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$group_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/therapy_groups/therapy_groups_views/groupDetailsParticipants.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$readonly might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/therapy_groups/therapy_groups_views/groupDetailsParticipants.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$statuses might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/therapy_groups/therapy_groups_views/groupDetailsParticipants.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$counselors might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/therapy_groups/therapy_groups_views/listGroups.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$deletion_response might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/therapy_groups/therapy_groups_views/listGroups.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$deletion_try might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/therapy_groups/therapy_groups_views/listGroups.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$group_types might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/therapy_groups/therapy_groups_views/listGroups.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$therapyGroups might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/therapy_groups/therapy_groups_views/listGroups.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$dbase might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../ippf_upgrade.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$this might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../library/ESign/views/default/esign_signature_log.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$this might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/ESign/views/encounter/esign_button.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$this might not be defined\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../library/ESign/views/encounter/esign_form.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$this might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../library/ESign/views/form/esign_button.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$this might not be defined\\.$#',
    'count' => 10,
    'path' => __DIR__ . '/../../library/ESign/views/form/esign_form.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$autojustify might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/FeeSheet.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$form_patient_id$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$form_patient_name$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$prog$#',
    'count' => 3,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$setting_selectors$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$userid$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$deletes might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$expired might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$facility might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$icon might not be defined\\.$#',
    'count' => 13,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$last_col_width might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$no_fu might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$no_interval might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid_list might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$provider might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ptkr_future_time might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tell_MedEx might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$today might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$urow might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result4 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/ajax/adminacl_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/ajax/adminacl_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$default$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/ajax/code_attributes_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$fileroot might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/ajax/code_attributes_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/ajax/dated_reminders_counter.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/ajax/easipro_util.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/ajax/facility_ajax_code.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$name_alt might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../library/ajax/graphs.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$titleGraphLine1 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/ajax/graphs.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$titleGraphLine2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/ajax/graphs.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/ajax/messages/validate_messages_document_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$bgcolor$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/ajax/payment_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$StringToAppend might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../library/ajax/payment_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$StringToAppend2 might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../library/ajax/payment_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$patient_code might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../library/ajax/payment_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$patient_code_complete might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/ajax/payment_ajax.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/ajax/set_pt.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/ajax/upload.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rtn might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/ajax/upload.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$provider$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/amc.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$pid$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/api.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$inline_arg might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/classes/Controller.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$string$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/classes/OFX.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$b2 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/classes/fpdf/fpdf.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$cmapkey might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/classes/fpdf/fpdf.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ls might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/classes/fpdf/fpdf.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ret might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/classes/postmaster.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sql might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/classes/rulesets/Amc/library/AbstractAmcReport.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$codes might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/classes/smtp/smtp.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$header_data might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/classes/smtp/smtp.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$new_file might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/classes/thumbnail/ThumbnailGenerator.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$types_support might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/classes/thumbnail/ThumbnailGenerator.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$excluded might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/clinical_rules.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pass_filter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/clinical_rules.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$patientNumber might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/clinical_rules.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$percentage might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/clinical_rules.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$temp_track_pass might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/clinical_rules.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$end might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/csv_like_join.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$st might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/custom_template/add_custombutton.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ISSUE_TYPES might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/custom_template/custom_template.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/custom_template/custom_template.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/custom_template/custom_template.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sel might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/custom_template/share_template.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$iter might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../library/daysheet.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$dn might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/deletedrug.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$web_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/dicom_frame.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$help_icon_title might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/display_help_icon_inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$dtp03$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_278_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ar01 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/edihistory/edih_278_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$dep_eb_html might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_278_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$elem03 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/edihistory/edih_278_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$elem05 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_278_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$loopid might not be defined\\.$#',
    'count' => 68,
    'path' => __DIR__ . '/../../library/edihistory/edih_278_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sbr_eb_html might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/edihistory/edih_278_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$cur03$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_835_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$cur04$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_835_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$n405$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/edihistory/edih_835_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ca might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_835_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$cd might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/edihistory/edih_835_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$cls might not be defined\\.$#',
    'count' => 34,
    'path' => __DIR__ . '/../../library/edihistory/edih_835_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$cq might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_835_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$cr might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_835_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$loopid might not be defined\\.$#',
    'count' => 38,
    'path' => __DIR__ . '/../../library/edihistory/edih_835_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$plb_amt might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_835_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$plb_rc might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_835_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$plb_rt might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_835_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$plb_tr might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_835_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$svcfee might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_835_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$svcpmt might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_835_html.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ackcode might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_997_error.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$acknote might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_997_error.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$fg_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_997_error.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$fg_type might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_997_error.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sub_icn might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_997_error.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$subdate might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_997_error.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$substn might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_997_error.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$subtime might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_997_error.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$subtype might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/edihistory/edih_997_error.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$file_type$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_archive.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$frow$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/edihistory/edih_archive.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$msg$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_archive.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$arch_fn might not be defined\\.$#',
    'count' => 12,
    'path' => __DIR__ . '/../../library/edihistory/edih_archive.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$archive_dir might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../library/edihistory/edih_archive.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$car2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_archive.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$car_cmb_unique might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../library/edihistory/edih_archive.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$h_ar might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_archive.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$isrn might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_archive.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$params might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_archive.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rnf might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_archive.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tmp_dir might not be defined\\.$#',
    'count' => 9,
    'path' => __DIR__ . '/../../library/edihistory/edih_archive.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$type_ar might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_archive.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$tp$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_data.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$dtpd2 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_data.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$dtpdlbl might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_data.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$errct might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_data.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$fn might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_data.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$trc might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_data.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$file_type$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$enc might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rws might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$rtp$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$bht03syn might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$bnfteq might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$cdx might not be defined\\.$#',
    'count' => 39,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$err_seg might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ft might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$gsdate might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$have_pt might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$hl might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$isrsp might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$payer_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$payer_name might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ptname might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rej_ct might not be defined\\.$#',
    'count' => 9,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rqst might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rspdate might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rspsttype might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rsptype might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sar might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$svcdate might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_csv_parse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$qs might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_io.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$str_html might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_io.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$has_eb might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_segments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$has_iii might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_segments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rspfile might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_segments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rspicn might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_segments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$stn might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../library/edihistory/edih_segments.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$err$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/edih_uploads.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$de$#',
    'count' => 18,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$delimiter3$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$ds$#',
    'count' => 4,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$bpr02 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$check_amount might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$check_date might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$chk might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ck might not be defined\\.$#',
    'count' => 52,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$de might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$gs_date might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$i might not be defined\\.$#',
    'count' => 27,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$loop might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$loopid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$lx01 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$out might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$plbar might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pmt_html might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$seg might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$segid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/edihistory/test_edih_835_accounting.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$current_state might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/expand_contract_inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$where might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../library/forms.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$CPR$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/ippf_issues.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/ippf_issues.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$datetimepicker_formatInput might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../library/js/xl/jquery-datetimepicker-2-5-4.js.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$datetimepicker_showseconds might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/js/xl/jquery-datetimepicker-2-5-4.js.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$datetimepicker_timepicker might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/js/xl/jquery-datetimepicker-2-5-4.js.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$disabled$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/options.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$lbfonchange$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/options.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$list_id_esc$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/options.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$under$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/options.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$count might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../library/options.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$group_name might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/options.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$lrow might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../library/options.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$metadata might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/options.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$insarr might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/patient.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$iter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/patient.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$returnval might not be defined\\.$#',
    'count' => 9,
    'path' => __DIR__ . '/../../library/patient.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AccountCode might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/payment.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$AdjustString might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/payment.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$all might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/registry.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tempCollectReminders might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/reminders.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$update_rem_log might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/reminders.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$where might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../library/reminders.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$arcount might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/report.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ret might not be defined\\.$#',
    'count' => 15,
    'path' => __DIR__ . '/../../library/report.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$retar might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/report.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pass_sql might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/report_database.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$_output might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/Smarty_Compiler_Legacy.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$arg_list might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/Smarty_Compiler_Legacy.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$attr_name might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/Smarty_Compiler_Legacy.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$compiled_ref might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/Smarty_Compiler_Legacy.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$expr might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/Smarty_Compiler_Legacy.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$i might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/Smarty_Compiler_Legacy.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$include_file might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/Smarty_Compiler_Legacy.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$last_token might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/Smarty_Compiler_Legacy.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$plugin_func might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/Smarty_Compiler_Legacy.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$return might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/Smarty_Compiler_Legacy.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$dummy$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/Smarty_Legacy.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$_debug_start_time might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/Smarty_Legacy.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$_included_tpls_idx might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/Smarty_Legacy.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$_smarty_results might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/Smarty_Legacy.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$debug_start_time might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/Smarty_Legacy.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$included_tpls_idx might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/Smarty_Legacy.class.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$_readable might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/internals/core.get_php_resource.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$_template_source might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/internals/core.get_php_resource.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$_message might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/internals/core.load_plugins.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$_plugin_func might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/internals/core.load_plugins.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$debug_start_time might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/internals/core.process_cached_inserts.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$_debug_start_time might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/internals/core.run_insert_handler.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$_config_vars might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/plugins/function.config_load.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$_debug_start_time might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/plugins/function.config_load.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$_var_compiled$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/plugins/function.eval.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/plugins/function.html_radios.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$day_values might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/plugins/function.html_select_date.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$minutes might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/plugins/function.html_select_time.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$seconds might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/plugins/function.html_select_time.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$loop$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/plugins/function.html_table.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ord might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/plugins/function.mailto.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$smarty might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/smarty_legacy/smarty/plugins/modifier.date_format.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/specialty_forms.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$bcodes might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/spreadsheet.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$cellstatic might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/spreadsheet.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/spreadsheet.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/spreadsheet.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$saveid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/spreadsheet.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$spreadsheet_form_name might not be defined\\.$#',
    'count' => 21,
    'path' => __DIR__ . '/../../library/spreadsheet.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$userauthorized might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/spreadsheet.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$handle1 might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/standard_tables_capture.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$new_str might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../library/standard_tables_capture.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/translation.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$use_validate_js might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/validation/validation_script.js.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$token_database might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../portal/account/account.lib.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$token_encrypt might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../portal/account/account.lib.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../portal/account/account.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$portalRegistrationAuthorization might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/account/register.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$hiddenLanguageField might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/account/verify.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result2 might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../portal/account/verify.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result3 might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../portal/account/verify.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$event_date might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/add_edit_event_user.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$insert might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/add_edit_event_user.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$patientname might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/add_edit_event_user.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$providers_current might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../portal/add_edit_event_user.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$starttime might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/add_edit_event_user.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$providerid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/find_appt_popup_user.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/get_allergies.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$class might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/get_amendments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/get_amendments.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/get_lab_results.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/get_medications.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/get_patient_documents.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/get_patient_info.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/get_prescriptions.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/get_pro.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/get_problems.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$insurance_data_array might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../portal/get_profile.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../portal/get_profile.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$landingpage might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/home.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../portal/home.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$defaultLangID might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/index.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$defaultLangName might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/index.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$one_time might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/index.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result2 might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../portal/index.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result3 might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../portal/index.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ccaudit might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/lib/paylib.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$invoice might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/lib/paylib.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$response might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../portal/lib/paylib.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$landingpage might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/logout.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/messaging/messages.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sn might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/patient/fwk/libs/util/parsecsv.lib.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$x might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../portal/patient/fwk/libs/util/parsecsv.lib.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$attachment$#',
    'count' => 4,
    'path' => __DIR__ . '/../../portal/patient/fwk/libs/verysimple/HTTP/FileUpload.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$output might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/patient/libs/Controller/PatientController.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$this might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../portal/patient/templates/DefaultError404.tpl.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$this might not be defined\\.$#',
    'count' => 9,
    'path' => __DIR__ . '/../../portal/patient/templates/DefaultErrorFatal.tpl.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$this might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../portal/patient/templates/OnsiteActivityViewListView.tpl.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$this might not be defined\\.$#',
    'count' => 14,
    'path' => __DIR__ . '/../../portal/patient/templates/OnsiteDocumentListView.tpl.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$this might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../portal/patient/templates/OnsitePortalActivityListView.tpl.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$this might not be defined\\.$#',
    'count' => 14,
    'path' => __DIR__ . '/../../portal/patient/templates/PatientListView.tpl.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$this might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../portal/patient/templates/ProviderHome.tpl.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$this might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../portal/patient/templates/_FormsHeader.tpl.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$this might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../portal/patient/templates/_Header.tpl.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$this might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../portal/patient/templates/_modalFormHeader.tpl.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$form_pid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../portal/portal_payment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$payrow might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/portal_payment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$adj_amt might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/report/pat_ledger.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$last_year might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../portal/report/pat_ledger.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pid might not be defined\\.$#',
    'count' => 10,
    'path' => __DIR__ . '/../../portal/report/pat_ledger.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_js_includes might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/report/pat_ledger.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ISSUE_TYPES might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/report/portal_custom_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OE_SITE_DIR might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/report/portal_custom_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$insurance_data_array might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../portal/report/portal_custom_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$prevIssueType might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/report/portal_custom_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_js_includes might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../portal/report/portal_custom_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webserver_root might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../portal/report/portal_custom_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ISSUE_TYPES might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/report/portal_patient_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rootdir might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../portal/report/portal_patient_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$srcdir might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../portal/report/portal_patient_report.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$OE_SITES_BASE might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../setup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$memory_limit_mb might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../setup.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$sqlconf might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../setup.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$pid$#',
    'count' => 1,
    'path' => __DIR__ . '/../../sites/default/LBF/LBFathbf.plugin.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$pid$#',
    'count' => 1,
    'path' => __DIR__ . '/../../sites/default/LBF/LBFvbf.plugin.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$age_index might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../sites/default/statement.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$desc might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../sites/default/statement.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$dun_message might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../sites/default/statement.inc.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$patientIdCc might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../sphere/initial_response.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$GLOBALS_METADATA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../sql_patch.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_database might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../sql_patch.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_major might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../sql_patch.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_minor might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../sql_patch.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_patch might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../sql_patch.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_realpatch might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../sql_patch.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_tag might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../sql_patch.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$GLOBALS_METADATA might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../sql_upgrade.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result2 might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../sql_upgrade.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_database might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../sql_upgrade.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_major might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../sql_upgrade.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_minor might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../sql_upgrade.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_patch might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../sql_upgrade.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$v_tag might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../sql_upgrade.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$webserver_root might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../sql_upgrade.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$iter might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../src/Billing/BillingReport.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$query_part_day might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../src/Billing/BillingReport.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$query_part_day1 might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../src/Billing/BillingReport.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$msp might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Billing/Claim.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$tmp might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Billing/Claim.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$elog might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Billing/EDI270.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$returnval might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Billing/EDI270.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$segment_ar$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Billing/EdiHistory/X12File.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$st_pos$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Billing/EdiHistory/X12File.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$bht_pos might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Billing/EdiHistory/X12File.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$dr might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Billing/EdiHistory/X12File.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ds might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Billing/EdiHistory/X12File.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$dt might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Billing/EdiHistory/X12File.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$env_ar might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../src/Billing/EdiHistory/X12File.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$gs_ct might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../src/Billing/EdiHistory/X12File.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$gs_fid might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Billing/EdiHistory/X12File.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$gs_start might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Billing/EdiHistory/X12File.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$gsn might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Billing/EdiHistory/X12File.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$icn might not be defined\\.$#',
    'count' => 10,
    'path' => __DIR__ . '/../../src/Billing/EdiHistory/X12File.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$seg_ar might not be defined\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../src/Billing/EdiHistory/X12File.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$segidx might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Billing/EdiHistory/X12File.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$stn might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Billing/EdiHistory/X12File.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$MDY might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Billing/Hcfa1500.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$allowed might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/ClinicalDecisionRules/Interface/Controller/ControllerEdit.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$columns might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/ClinicalDecisionRules/Interface/Controller/ControllerEdit.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$criteria might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../src/ClinicalDecisionRules/Interface/Controller/ControllerEdit.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$arr_group_titles might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Common/Acl/AclExtended.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$response might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Common/Auth/MfaUtils.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$issues might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Common/Command/CreateReleaseChangelogCommand.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$fhirScopes might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Common/Command/GenerateAccessTokenCommand.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$values might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Common/ORDataObject/ORDataObject.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$row$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Easipro/Easipro.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$returnval might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Easipro/Easipro.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$retarr might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Gacl/Gacl.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$row_count$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Gacl/GaclApi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$groups_map_table might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Gacl/GaclApi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$object_group_table might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Gacl/GaclApi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$object_map_table might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Gacl/GaclApi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$object_sections_table might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../src/Gacl/GaclApi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$retarr might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Gacl/GaclApi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$table might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Gacl/GaclApi.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$catEntry might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../src/Menu/MainMenuRole.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$action might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/OeUI/OemrUI.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$action_href might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/OeUI/OemrUI.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$action_title might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../src/OeUI/OemrUI.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$frontSpecificRetail might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/PaymentProcessing/Sphere/SpherePayment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$trxcustidRetail might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/PaymentProcessing/Sphere/SpherePayment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$trxcustidRetailLicensekey might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/PaymentProcessing/Sphere/SpherePayment.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$zip might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Pharmacy/Services/ImportPharmacies.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$exportWriter might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/RestControllers/FHIR/Operations/FhirOperationExportRestController.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$outputResult might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/RestControllers/FHIR/Operations/FhirOperationExportRestController.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$resource might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/RestControllers/FHIR/Operations/FhirOperationExportRestController.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$all might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Rx/RxList.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ending might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Rx/RxList.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$list might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Rx/RxList.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$my_data might not be defined\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../src/Rx/RxList.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$type might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../src/Rx/RxList.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$ct_id might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$date might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$enc_date might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$enc_id might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter_for_forms might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$encounter_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$end_date might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$facility_id might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$id_data might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$immunization_date_value might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$list_id might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$med_pblm_begdate_value might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$med_pblm_enddate_value might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$o_id might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$oid_unit might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$pro_id might not be defined\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$provider_id might not be defined\\.$#',
    'count' => 10,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$res3 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$res6 might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$res_q_sel_allergies might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$res_q_sel_encounter might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$res_q_sel_imm might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$res_q_sel_med_pblm might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$res_q_sel_vitals might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$res_query_sel_fac might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$vitals_date_value might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$vitals_id might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateImportDispose.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaTemplateParse.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$attributes might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/DecisionSupportInterventionService.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$service might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Services/DecisionSupportInterventionService.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$q_id might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/DocumentTemplates/DocumentTemplateRender.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$group_list might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/DocumentTemplates/DocumentTemplateService.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$rtn might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/DocumentTemplates/DocumentTemplateService.php',
];
$ignoreErrors[] = [
    'message' => '#^Undefined variable\\: \\$puuid$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/DrugService.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$code might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/FHIR/FhirImmunizationService.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$display might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/FHIR/FhirImmunizationService.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$class_object might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/FHIR/FhirResourcesService.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$result might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/InsuranceCompanyService.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$return might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/LogoService.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$days might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/PatientTrackerService.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$hours might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/PatientTrackerService.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$minutes might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/PatientTrackerService.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$returnval might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/PatientTrackerService.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$q_url might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/QuestionnaireService.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$q_uuid might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/QuestionnaireService.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$records might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/Utils/SQLUpgradeService.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$viewBean might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../templates/super/rules/base/template/basic.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$viewBean might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../templates/super/rules/base/template/criteria.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$_redirect might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../templates/super/rules/base/template/redirect.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$viewBean might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../templates/super/rules/controllers/alerts/list_actmgr.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$viewBean might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../templates/super/rules/controllers/detail/view.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$viewBean might not be defined\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../templates/super/rules/controllers/edit/action.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$viewBean might not be defined\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../templates/super/rules/controllers/edit/add_criteria.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$criteria might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../templates/super/rules/controllers/edit/age.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$criteria might not be defined\\.$#',
    'count' => 14,
    'path' => __DIR__ . '/../../templates/super/rules/controllers/edit/bucket.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$criteria might not be defined\\.$#',
    'count' => 18,
    'path' => __DIR__ . '/../../templates/super/rules/controllers/edit/custom.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$criteria might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../templates/super/rules/controllers/edit/diagnosis.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$viewBean might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../templates/super/rules/controllers/edit/intervals.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$criteria might not be defined\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../templates/super/rules/controllers/edit/lifestyle.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$criteria might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../templates/super/rules/controllers/edit/sex.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$criteria might not be defined\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../templates/super/rules/controllers/edit/simple_text_criteria.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$viewBean might not be defined\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../templates/super/rules/controllers/edit/summary.php',
];
$ignoreErrors[] = [
    'message' => '#^Variable \\$viewBean might not be defined\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../templates/super/rules/controllers/review/view.php',
];

return ['parameters' => ['ignoreErrors' => $ignoreErrors]];
